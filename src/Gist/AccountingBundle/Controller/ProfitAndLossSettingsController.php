...
class ProfitAndLossSettingsController extends TrialBalanceController
{
    use TrackCreate;
    ...
    {
        $this->route_prefix = 'gist_pnl_settings';
        $this->title = 'Profit or Loss Settings';
        $this->list_title = 'Profit or Loss Settings';
    ...
    public function indexAction()
    {
        ...
        $twig_file = 'GistAccountingBundle:ProfitAndLossSettings:index.html.twig';
    ...
    public function saveAction()
    {
        try
        {
            ...
            $cfs = $em->getRepository('GistAccountingBundle:ProfitAndLossSettings')->findAll();
            foreach ($cfs as $cf) {
                $em->remove($cf);
            }
            $em->flush();
            ...
            if(isset($data['sales'])) {
                foreach ($data['sales'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_SALES)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['return'])) {
                foreach ($data['return'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_RETURN)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['sales_return'])) {
                foreach ($data['sales_return'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_SR)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['cos'])) {
                foreach ($data['cos'] as $key => $account) {
                    ...
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_COS)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['col'])) {
                foreach ($data['col'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_COL)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['oec'])) {
                foreach ($data['oec'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_OEC)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['uc'])) {
                foreach ($data['uc'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_UC)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['tcdc'])) {
                foreach ($data['tcdc'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_TCDC)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['supp'])) {
                foreach ($data['supp'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_SUPP)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['rm'])) {
                foreach ($data['rm'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_RM)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['foo'])) {
                foreach ($data['foo'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_FOO)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['oee'])) {
                foreach ($data['oee'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_OEE)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['oc'])) {
                foreach ($data['oc'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_OC)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['da'])) {
                foreach ($data['da'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_DA)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['mab'])) {
                foreach ($data['mab'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_MAB)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['oic'])) {
                foreach ($data['oic'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_OIC)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            if(isset($data['nit'])) {
                foreach ($data['nit'] as $key => $account) {
                    $id = $am->findChartOfAccount($account);
                    $pnl = new ProfitAndLossSettings();
                    $pnl->setAccount($id)
                        ->setType(ProfitAndLossSettings::TYPE_NIT)
                        ->setUserCreate($this->getUser());
                    $em->persist($pnl);
                }
            }
            ...
            return $this->redirect($this->generateUrl('gist_pnl_settings_index'));
        }
    ...
    protected function padListParams(&$params, $obj = null)
    {
        ...
        $params['sales_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_SALES);
        $params['return_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_RETURN);
        $params['sr_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_SR);
        $params['cos_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_COS);
        $params['col_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_COL);
        $params['oec_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_OEC);
        $params['uc_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_UC);
        $params['tcdc_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_TCDC);
        $params['supp_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_SUPP);
        $params['rm_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_RM);
        $params['foo_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_FOO);
        $params['oee_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_OEE);
        $params['oc_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_OC);
        $params['da_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_DA);
        $params['mab_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_MAB);
        $params['oic_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_OIC);
        $params['nit_opts_selected'] = $am->findProfitAndLossByType(ProfitAndLossSettings::TYPE_NIT);
