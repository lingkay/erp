...
class PendingSchedulesController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null)
    {
        ...
        $this->print = 'gist_report_pending_print';
        $this->csv = 'gist_report_pending_csv';
        $this->filter = 'gist_report_pending_summary_filter';

        // get params
        $params = $this->getViewParams('', 'gist_report_pending_summary');
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = 'select s from GistServiceBundle:ServiceOrder s where s.date_issue >= :date_from and s.date_issue <= :date_to and s.status_id = :issued';
