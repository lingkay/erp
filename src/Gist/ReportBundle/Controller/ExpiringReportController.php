...
class ExpiringReportController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null)
    {
        ...
        $this->print = 'gist_report_expiring_print';
        $this->csv = 'gist_report_expiring_csv';
        $this->filter = 'gist_report_expiring_summary_filter';

        // get params
        $params = $this->getViewParams('', 'gist_report_expiring_summary');
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = 'select s from GistServiceBundle:ServiceOrder s where s.date_expire >= :date_from and s.date_expire <= :date_to';
