...
class ServiceSummaryController extends BaseController
{
    public function indexAction($date_from = null , $date_to = null)
    {
        ...
        $this->print = 'gist_report_service_print';
        $this->csv = 'gist_report_service_csv';
        $this->filter = 'gist_report_service_summary_filter';
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = 'select s from GistServiceBundle:ServiceOrder s where s.date_issue >= :date_from and s.date_issue <= :date_to';
