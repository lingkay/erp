...
use Gist\InventoryBundle\Entity\BorrowedTransaction;
...
use Gist\InventoryBundle\Entity\ReturnedItem;
...
class BorrowedTransactionController extends CrudController
{
    ...
    public function index2Action(Request $request)
    {
        ...
        if ($request->get('department')) {
            $params['selected_dept'] = $request->get('department');
            $department = $request->get('department');
        }

        if ($request->get('category')) {
            $params['selected_category'] = $request->get('category');
        ...
        /*$params['selected_dept'] = 'test';
        $department = 'test department';    

        $params['selected_category'] = 'test';
        $category = 'test';*/

        $twig_file = 'GistInventoryBundle:BorrowedTransaction:index.html.twig';
        ...
        $params['selected_dept'] = $department;

        //fetch the data based on date range
        $em = $this->getDoctrine()->getManager();
        //$borrowed = $em->getRepository('GistInventoryBundle:BorrowedTransaction')->findAll();
        $params['data'] = $this->getBorrowCreated($date_from,$date_to, $department, $category);
    ...
    public function printGridAction($date_from, $date_to, $category, $department)
    {
        $em = $this->getDoctrine()->getManager();
        ...
        $date_from = new DateTime($date_from->format("Y-m-d")." 00:00:00");
        $date_to =  new DateTime($date_to->format("Y-m-d")." 23:59:59");
        $data = $this->getBorrowCreated($date_from, $date_to, $department, $category);
        
        $params['data'] = $data;

       
        return $this->render('GistInventoryBundle:BorrowedTransaction:print_grid.html.twig', $params);
    }

    public function gridGetBorrowed($id)
    {
        $em = $this->getDoctrine()->getManager();
        $obj = $em
               ->getRepository('GistInventoryBundle:BorrowedTransaction')
               ->findBy(
                     array('id' => $id)
                        );
        ...
        if ($obj) 
        {
            foreach ($obj as $o) 
            {
                if ($o->getEntries()) 
                {
                    ...
                    {
                        $list .= 'Product Name: '.$entry->getProduct()->getName().'<br>'.'Quantity: '.$entry->getQuantity();
                    }
                }
            }
        }
    ...
    public function getDeptAction($id)
    {
        ...
        $user = $em->getRepository('GistUserBundle:User')->findAll();
    ...
    public function getProductAction($prod_id)
    {
        ...
        $user = $em->getRepository('GistInventoryBundle:Product')->findAll();
    ...
    protected function padFormParams(&$params, $date_from = null, $date_to = null)
    {
        ...
        $products = $em->getRepository('GistInventoryBundle:Product')
        ...
        foreach ($depts as $dept)
            $dept_opts[$dept->getName()] = $dept->getName();
        ...
        $params['equipment_warehouse'] = $config->get('gist_warehouse_equipment');

        $params['status_opts'] = array(
            'Borrowed'=> BorrowedTransaction::STATUS_BORROWED, 
            'Returned'=> BorrowedTransaction::STATUS_RETURNED
        );        
    ...
    public function filterAction($date_from, $date_to)
    {
        ...
        return $this->render('GistInventoryBundle:BorrowedTransaction:index.html.twig', $params);
    }

    public function getBorrowCreated($date_from, $date_to, $department, $category)
    {
        $where_dept = '';
        $where_cat = '';
        if ($department != '') 
        {
            $where_dept = 'AND g.name= :department ';
            $where_cat = 'WHERE pg.name=:category';
        }
        
        $em = $this->getDoctrine()->getManager();
        $query = $em->createQuery('SELECT b FROM GistInventoryBundle:BorrowedTransaction b
                    INNER JOIN b.user_create u
                    INNER JOIN u.dept g
                    WHERE b.date_create >= :date_from AND b.date_create <= :date_to '.$where_dept.'
                    AND b.id IN (SELECT t.id FROM GistInventoryBundle:BIEntry x 
                        INNER JOIN x.borrowed t
                        INNER JOIN x.product p
                        INNER JOIN p.prodgroup pg
                        '.$where_cat.')');
        ...
        $borrow_created = $query->getResult();

        $borrow_arr = array();

        foreach($borrow_created as $bc) {
            $borrow_arr[] = [
                'code' => $bc->getCode(),
                'department' => $bc->getBorrower()->getDepartment()->getName(),
                'borrowed_items' => $this->gridGetBorrowed($bc->getID()),  
                'date_create' => $bc->getDateCreateFormatted(),
                'created_by' => $bc->getUserCreate()->getName(),
                'status' => $bc->getStatus(), 
                'id' => $bc->getID(), 
            ] ; 
    ...
    {            
        ...
        $wh_acc = $this->getEquipmentWarehouseAccount();
        ...
        if(isset($data['prod_opts']))
        {
            foreach ($data['prod_opts'] as $index => $prod_id) 
            {
                ...
                $entry = $em->getRepository('GistInventoryBundle:BIEntry')->find($id);
                if ($entry == null)
                {
                    $entry = new BIEntry();
    ...
    {            
        ...
        if(isset($data['date_returned']))
        {
            ...
            {
                $entry = $em->getRepository('GistInventoryBundle:BIEntry')->find($data['entry_id'][$index]);
                ...
                $prod = $em->getRepository('GistInventoryBundle:Product')->find($data['prod_id'][$index]); 
    ...
    public function addSubmitAction()
    {
        ...
        try
        {
            ...
            if ($checker == true)
            {
                $this->addFlash('error', 'Not enough stock in equipment warehouse.');
                return $this->redirect($url);
            }
            else
            {
    ...
    public function editSubmitAction($id)
    {
        ...
        try
        {
            ...
            // }
            if($returned_validator == true)
            {
                $this->addFlash('error', 'Return item exceeded');
                return $this->redirect($this->generateUrl($this->getRouteGen()->getEdit(), array('id' => $id)));
            }
            else
            {
                ...
                {
                    $object->setStatus(BorrowedTransaction::STATUS_RETURNED);
                }
                else
                {
                    $object->setStatus(BorrowedTransaction::STATUS_BORROWED);
                }
    ...
    {            
        ...
        if(isset($data['prod_opts']))
        {
            foreach ($data['prod_opts'] as $index => $prod_id) 
            {
                ...
                if (intval($qty) > intval($stock_count) or $stock_count == null)
                {
                    if (intval($qty) == intval($stock_count)) 
                    {
                        $checker = false;
                    }
                    else
        ...
        //die(intval($stock_count).' < '.$qty.'<br>'.$wh_acc->getName().'-'.$prod->getName() .'<br>Checker: '.$checker);
        return $checker;
    }
    ...
    public function deleteEntryAction($borrow_id, $entry_id)
    {
        ...
        $entry = $em->getRepository('GistInventoryBundle:BIEntry')->find($entry_id);
    ...
    protected function getEquipmentWarehouseAccount()
    {            
        $em = $this->getDoctrine()->getManager();

        $config = $this->get('gist_configuration');
        $main_warehouse = $config->get('gist_warehouse_equipment');
        $wh = $em->getRepository('GistInventoryBundle:Warehouse')->find($main_warehouse);
    ...
    public function printPDFAction($id)
    {
        ...
        $borrowed = $em->getRepository('GistInventoryBundle:BIEntry')->findBy(array('borrowed' => $id ));
        ...
        $html = $this->render('GistInventoryBundle:BorrowedTransaction:print.html.twig', $params);
    ...
    public function getStockReport()
    {
        ...
        $query = $em->createQuery('SELECT o.code, d.name, LOWER(o.date_issue), o.status  
            FROM GistInventoryBundle:BorrowedTransaction o 
