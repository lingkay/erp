...
class CountingController extends CrudController
{
    ...
    {
        $twig = "GistInventoryBundle:Counting:print.html.twig";
    ...
    private function getOutputData($id)
    {
        ...
        $query->from('GistInventoryBundle:Counting', 'o');
    ...
    public function getSentFromPOSAction($pos_loc_id)
    {
        ...
        $stock_transfers = $em->getRepository('GistInventoryBundle:Counting')->findBy(array('source_inv_account'=>$pos_location->getInventoryAccount()->getID()));
    ...
    public function getSentToPOSAction($pos_loc_id)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $pos_location = $em->getRepository('GistLocationBundle:POSLocations')->findOneBy(array('id'=>$pos_loc_id));
        $stock_transfers = $em->getRepository('GistInventoryBundle:Counting')->findBy(array('destination_inv_account'=>$pos_location->getInventoryAccount()->getID()));
     ...
     * @internal param $pos_loc_id
     */
    public function updatePOSCountingAction($id, $userId, $status, $entries)
    {
        header("Access-Control-Allow-Origin: *");
        parse_str($entries, $entriesParsed);
        $stockManipulationManager = $this->get('gist_stock_manipulation_manager');
        $em = $this->getDoctrine()->getManager();
        $st = $em->getRepository('GistInventoryBundle:Counting')->findOneBy(array('id'=>$id));
        ...
        if($status == 'processed') {
            ...
            $inv_stock_transfer->updateCountingEntriesProcessed($entriesParsed, $st);
            ...
            $inv_stock_transfer->updateCountingEntriesArrived($entriesParsed, $st);
            $stockManipulationManager->performTransfer($entriesParsed, $st, $user, 'POS');

        } elseif ($status == 'requested') {
            $inv_stock_transfer->removeEntries($st);
            $inv_stock_transfer->updateCountingEntriesRequested($entriesParsed, $st);
        } else {
            $list_opts[] = array(
                'status'=>'failed'
            );
    ...
    public function updatePOSCountingFormAction($user, $entries, $id)
    {
        $inv_stock_transfer = $this->get('gist_inventory_stock_transfer');
     ...
     * @internal param $pos_loc_id
     */
    public function addPOSCountingAction($src, $dest, $user, $description, $entries, $status, $id)
    {
        header("Access-Control-Allow-Origin: *");
        $em = $this->getDoctrine()->getManager();
        $inv = $this->get('gist_inventory');
        ...
        if ($status != 'to_update') {
            ...
            foreach ($entriesParsed as $e) {
                ...
                $entry = new CountingEntry();
                $entry->setCounting($st)
                    ->setProduct($prod)
                    ->setQuantity($qty);
            ...
            $transferStock = $em->getRepository('GistInventoryBundle:Counting')->findOneBy(array('id'=>$id));
            $existingEntries = $em->getRepository('GistInventoryBundle:CountingEntry')->findBy(array('stock_transfer'=>$transferStock->getID()));
            ...
            foreach ($entriesParsed as $e) {
                ...
                $entry = new CountingEntry();
                $entry->setCounting($transferStock)
                    ->setProduct($prod)
                    ->setQuantity($qty);
