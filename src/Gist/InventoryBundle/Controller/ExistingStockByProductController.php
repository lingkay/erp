...
class ExistingStockByProductController extends Controller
{
    protected $repo;
    protected $base_view;
    protected $route_gen;
    ...
    public function indexAction($pos_loc_id = null, $inv_type = null, $date_from = null, $date_to = null, $product_name = null)
    {
        if ($product_name == 'null') {
            $product_name = null;
        }
        $inv = $this->get('gist_inventory');
        $this->route_prefix = 'gist_inv_existing_stock_by_product';
        $this->title = 'Existing Stock by Product';
        ...
        $params['grid_cols'] = $gl->getColumns();
        $params['es_data'] = $this->getExistingStockData($pos_loc_id, $inv_type, $date_from, $date_to, $product_name);
        $params['selected_inv_type'] = $inv_type;

        $params['product_name'] = $product_name;


        $twig_file = 'GistInventoryBundle:ExistingStockByProduct:index.html.twig';
...
//        $date_to = DateTime::createFromFormat('Ymd', $date_to->format('Ymd'));
        ...
        if ($quantitySold == 0) {
            ...
            $avgConsumption = number_format($quantitySold/$days, 3);
