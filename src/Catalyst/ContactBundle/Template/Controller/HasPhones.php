...
trait HasPhones
{
    protected function updateHasPhones($o, $data, $is_new)
    {
        ...
        if(isset($data['phone_id'])){
            ...
            {
                // find phone object
                $phone = $em->getRepository('CatalystContactBundle:Phone')->find($add_id);
        ...
        foreach ($contacts as $contact) {
            if(!in_array($contact->getId(), $phones)) {
                $o->removePhone($em->getRepository('CatalystContactBundle:Phone')->find($contact->getId()));
                $em->remove($em->getRepository('CatalystContactBundle:Phone')->find($contact->getId()));
            }
        }
    }
