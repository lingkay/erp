...
class AdjustmentController extends CrudController
{
    ...
    protected function getData()
    {
        ...
        $qb->select('o')
            ->from('HrisToolsBundle:EmployeeAdjustment', 'o')
            ->where('o.date_adjustment between :date_from and :date_to ')
            ->setParameter('date_from', $this->date_from)
            ->setParameter('date_to', $this->date_to);
    ...
    public function csvAction()
    {
        ...
        foreach ($this->getData() as $row) {
           ...
           $data[] = $row->getAmount();
           $data[] = $row->getAmount();
           $data[] = $row->getAdjustmentType();
           $data[] = $row->getNotes();
           $data[] = $row->getDateAdjustment()->format('m/d/Y');
           $data[] = $row->getDateAdjustment()->format('F');
           $data[] = $row->getCutoff();
           $data[] = $row->getDateAdjustment()->format('Y');
